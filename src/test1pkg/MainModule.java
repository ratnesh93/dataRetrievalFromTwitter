...
public class MainModule {
	...
	static final String DB_URL = "jdbc:mysql://192.168.112.12/mydbv2";
	...
	public static void main(String[] args)
			throws TwitterException, SQLException, ClassNotFoundException, UnsupportedEncodingException {
		...
		if (query_no == 1) {
			...
			// queryStr = title + place; //remove stopwords ka code banana
			// hai....get a list of stopwords!
		} else if (query_no == 4) {
			...
			// queryStr = title; //remove stopwords ka code banana hai....get a
			// list of stopwords!
		} else if (query_no == 9) {
		...
		while (searchResultCount != 0) {
			...
			for (Status status : result.getTweets()) {
				...
				if (!rs.next()) {
					...
					// User newUser=new
					// User(status.getUser().getId(),status.getUser().getDescription(),
					// status.getUser().getFollowersCount(),
					// status.getUser().getFriendsCount(), new
					// java.sql.Date(status.getUser().getCreatedAt().getTime()),
					// status.getUser().getLang(),
					// status.getUser().getLocation(),
					// status.getUser().getName()
					// ,status.getUser().getScreenName(),
					// status.getUser().getStatusesCount(),status.getUser().isVerified());
					// newUser.insertUser();
				...
				sql = "SELECT * from tweet where id=" + status.getId(); // perform
																		// and
																		// query
																		// here
																		// with
				...
				if (!rs.next()) {
					...
					// ps.setString(3, new
					// String(status.getText().getBytes("UTF-8")));
